...
<!-- <div class="row"> -->
            <!-- <div> -->
                ...
                <div class="col-8" id="registration"><!--Registration form-->
                        ...
                        <?php  if (isset($_REQUEST['action'])&& $_REQUEST['action']=='add_topics'){
    ...
    <div class="card-header bg-info text-secondary">
        ...
        <div class="mb-3">
         <label for="batch" class="form-label">Select Topics</label>
         <!-- <span class=""  id="msgbatch" style="color: red;">*</span> -->
           <?php
                $num_of_topics = $_REQUEST['number_of_topics'];
             for ($i=1; $i<=$num_of_topics; $i++) { 
                $select_topic = $dobj->select_topic();
             ?>
        <select class="form-control" id="select_topic" name="select_topic[]">
            <option>Select topics</option>
             <?php while($topic = mysqli_fetch_assoc($select_topic)){
             // if ($select_topic->num_rows>=$_REQUEST['number_of_topics']){
            ?>
            <option name="topic_id" value="<?php echo $topic['topic_id']; ?>" required>
            <?php
            // var_dump($topic);
            // die;
            echo $topic['topic_title']." ".$topic['topic_description'];
            ?>
            </option>
        <?php 
            }
...
<!--          <label for="select_course" class="form-label">Select course</label>
        ...
        ?>
        <option name='course_id' value="<?php //echo $course['course_id']; ?>">
        <?php // echo $course['course_title']." ".$course['course_description'];?>
        </option>
        <?php // } ?>
        </select> -->
         <label for="priority" class="form-label">Select topic priority</label>
         <!-- <span class=""  id="msgtopicpriorty" style="color: red;">*</span> -->
         <input type ="number" class="form-control mb-2" id="priority" name="priority[]" required="true">
             <?php 
         ...
         <!-- <input type ="date" class="form-control" id="course_start_date" name="course_start_date" required="true"></textarea> -->
                <center>    
        <input type="hidden" name="action" value="assign_topics">
        <input type="hidden" name="batch_course_id" value="<?php echo $_REQUEST['batch_course_id']; ?>">
        <button type="submit" class="btn btn-secondary m-1" name="assign_topics">Add Topics </button></center>
        </div>
        </div>
    </form>
    </div>
