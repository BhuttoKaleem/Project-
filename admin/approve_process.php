...
$result = $dobj->approve($_REQUEST['user_id'],$updated_at,$_SESSION['user']['user_id']);
if ($result){
    header("location:approve_users.php?class=success&msg=Approved");
}
...
$message = wordwrap("Your account is activated, now you can login to your account using credentials used",70);
...
if (!$mail->send()) {
    ...
    header("location:approve_users.php?class=success&msg=Approved and mail sent at user ID".$_REQUEST['user_id']);
}
