...
if (isset($_REQUEST['upload'])&&$_REQUEST['action']=='upload_topic_file') {
	...
	print_r($_FILES);
	// die;
	$get_topic_file = $dobj->select_topic_file($_REQUEST['topic_id'],$_FILES['topic_file']['name']);
	if($get_topic_file->num_rows){
		header("location:course_view.php?class=danger&msg=File already available&batch_course_id=".$_REQUEST['batch_course_id']);
		die;
	}
	...
	echo $file_extension;
	if (!($file_extension =='ppt'|| $file_extension=='doc'|| $file_extension=='pdf' || $file_extension=='jpg'||$file_extension=='jpeg' || $file_extension=='png' || $file_extension=='pptx')) {
	...
	if ($_FILES['topic_file']['size']>2000000) {
		header("location:course_view.php?class=danger&msg=File size must be 2MB&batch_course_id=".$_REQUEST['batch_course_id']);
		die;
	}
	echo $file_name."<br/>";
	echo $file_path."<br/>";
...
if (isset($_REQUEST['action'])&&$_REQUEST['action']=='update_file') {
	$file_update = $dobj->update_file($_REQUEST['topic_file_id'],$_REQUEST['status_id'],$_REQUEST['topic_id']);
	if ($file_update) {
